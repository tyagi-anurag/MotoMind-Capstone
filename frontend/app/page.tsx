    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white font-sans overflow-hidden relative flex flex-col md:flex-row">
      ...
      <AnimatePresence>
        {showOnboarding && (
          ...
          >
            <motion.div 
              className="bg-[#121212] border border-white/10 rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl relative overflow-hidden"
                ...
                <div>
                  ...
                  <div className="relative mt-2">
                    ...
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl p-3 pl-10 text-white focus:border-cyan-500 outline-none transition-colors"
                    />
                  </div>
                </div>
                ...
                <div className="pt-2"> 
                  ...
                  <div className="mt-3 space-y-3">
                    <button 
                        ...
                        disabled={isLocating}
                        className="w-full py-3 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 rounded-xl border border-cyan-500/50 flex items-center justify-center gap-2 transition-all text-sm font-bold shadow-lg shadow-cyan-900/20"
                    >
                    ...
                    <div className="relative">
                        ...
                        <input 
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-xl p-3 pl-10 text-white focus:border-cyan-500 outline-none transition-colors placeholder:text-gray-600"
                        />
                    </div>
                  </div>
                </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input 
              value={bikeModel} 
              onChange={(e) => setBikeModel(e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-lg p-2 text-sm mt-1 focus:border-cyan-500 outline-none transition-colors"
            />
          </div>
          ...
          <div>
            ...
            <div className="mt-2 space-y-2">
                <button 
                    onClick={handleAutoLocation} 
                    disabled={isLocating}
                    className="w-full py-2 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 rounded-lg border border-cyan-500/50 flex items-center justify-center gap-2 transition-all text-sm font-bold"
                >
                ...
                <div className="relative">
                    ...
                    <input 
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-lg p-2 pl-9 text-sm focus:border-cyan-500 outline-none transition-colors placeholder:text-gray-600"
                    />
                </div>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col relative z-10 h-screen w-full">
        ...
        <header className="px-4 md:px-6 py-4 border-b border-white/5 flex items-center gap-4 md:gap-6 bg-black/20 backdrop-blur-sm overflow-x-auto scrollbar-hide">
          ...
          {[
            ...
            { id: "travel", label: "Trip Planner", icon: Map },
            { id: "rescue", label: "Rescue Map", icon: Wrench },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-2 text-sm font-medium pb-1 transition-all border-b-2 whitespace-nowrap ${
                activeTab === tab.id 
            ...
            <>
              <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 scrollbar-thin scrollbar-thumb-white/10 pt-8 md:pt-12">
                {messages.map((msg, i) => (
                  ...
                  >
                    <div className={`max-w-[90%] md:max-w-[85%] p-4 rounded-2xl border backdrop-blur-sm shadow-xl ${
                      msg.role === "user" 
                ...
                <AnimatePresence>
                  {attachedFile && (
                    <motion.div 
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: 10 }}
                      className="mb-3 flex items-center gap-3 bg-white/10 w-fit px-4 py-2 rounded-full border border-white/10"
                    >
                ...
                {!tripResult ? (
                    <div className="bg-white/5 p-6 rounded-2xl border border-white/10 space-y-6 shadow-2xl">
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div><label className="text-xs text-gray-500 uppercase font-bold">From</label><input id="trip-from" placeholder="e.g. Delhi" className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500 outline-none" /></div>
                            <div><label className="text-xs text-gray-500 uppercase font-bold">To</label><input id="trip-to" placeholder="e.g. Ladakh" className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500 outline-none" /></div>
                         </div>
                         <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="md:col-span-2"><label className="text-xs text-gray-500 uppercase font-bold">Bike</label><input id="trip-bike" defaultValue={bikeModel} className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500 outline-none" /></div>
                            <div className="grid grid-cols-2 gap-4 md:contents">
                                <div><label className="text-xs text-gray-500 uppercase font-bold">Days</label><input id="trip-days" type="number" placeholder="10" className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500 outline-none" /></div>
                                <div><label className="text-xs text-gray-500 uppercase font-bold">Riders</label><input id="trip-people" type="number" defaultValue="1" className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white focus:border-cyan-500 outline-none" /></div>
                            </div>
                         </div>
